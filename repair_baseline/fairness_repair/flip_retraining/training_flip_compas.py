...
for benchmark in ['r','g','r&g']:
    ...
    #             'r&g': [-0.9, 0.8],
    # }
    X_train = pre_compas_scores.X_train
    y_train = pre_compas_scores.y_train
    constraint = pre_compas_scores.constraint
    ...
    history = model.fit(X_shuffled, y_shuffled, epochs=30, validation_data=(pre_compas_scores.X_val, pre_compas_scores.y_val))
    model.evaluate(pre_compas_scores.X_test, pre_compas_scores.y_test) # 84.32% accuracy
    model.save("models/models_flip/compas_model_"+benchmark+".h5")
    # The precision rate is  0.7338425381903643 , the recall rate is  0.5454148471615721 , and the F1 score is  0.625751503006012
    # loss: 0.6317 - accuracy: 0.6468