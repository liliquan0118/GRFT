...
from models_torch_lsac import MultiTaskModel as Net 
...
def run(train_batch_size, val_batch_size, epochs, lr, momentum, log_interval, log_dir,args):
    ...
    sex_y_train = expected_dataset.X_train[:,10].copy()
    sex_y_val = expected_dataset.X_val[:,10].copy()
    sex_y_test = expected_dataset.X_test[:,10].copy()

    race_y_train = expected_dataset.X_train[:,9].copy()
    race_y_val = expected_dataset.X_val[:,9].copy()
    race_y_test = expected_dataset.X_test[:,9].copy()
    ...
    if is_mask_gender:
        ...
        X_val[:,7]=0
        X_test[:,7]=0
    print ("sex_val",np.unique(X_val[:,10]),
           "sex_train",np.unique(X_train[:,10]),
    ...
    convert_keras_model.save("models/multitask_models/lsac.h5")
    print ("=======")
    print ("========fairness==")
    import sys
    import hashlib
...
if __name__ == "__main__":
    ...
    elif args.dataset=="pre_lsac":
        from preprocessing import pre_lsac as expected_dataset
