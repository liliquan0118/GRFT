...
from models_torch_bank import MultiTaskModel as Net 
...
def eval_dataset(model,val_loader,name="val"):
    ...
    total = 0
    with torch.no_grad():
        for i,(x,real_class,real_class_age) in enumerate(val_loader):
            test_X = x.to(device)
...
def run(train_batch_size, val_batch_size, epochs, lr, momentum, log_interval, log_dir,args):
    ...
    for epoch in tqdm(range(epochs),total=epochs,desc="train"):
        ...
        for i,(x,y,y_age) in enumerate(train_loader):
            x= x.to(device)
            ...
            loss = larm_a * loss1 + larm_b_age * loss_age 
            loss.backward()
            optimizer.step() 
        
            loss_collect.append([loss1.item(),loss_age.item()])
        ...
        if epoch%5==0 or epoch==epochs-1:
            ...
            loss_collect_dict =  zip(loss_collect,["task","age"][:len(loss_collect)] )
    ...
    model_keras = keras.Sequential([
        keras.layers.Dense(30, activation="relu", input_shape=expected_dataset.X_train.shape[1:]),
    ...
    convert_keras_model.save("models/multitask_models/bank.h5")
    print ("=======")
    print ("========fairness==")
    import sys
    import hashlib
...
if __name__ == "__main__":
    ...
    parser.add_argument(
     "--dataset", default="pre_bank_marketing" ,
        )
