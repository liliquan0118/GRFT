...
from models_torch_scores import MultiTaskModel as Net 
...
def eval_dataset(model,val_loader,name="val"):
    ...
    total = 0
    with torch.no_grad():
        for i,(x,real_class,real_class_sex,real_class_race) in enumerate(val_loader):
            test_X = x.to(device)
...
def run(train_batch_size, val_batch_size, epochs, lr, momentum, log_interval, log_dir,args):
    ...
    sex_y_train = expected_dataset.X_train[:,5].copy()
    sex_y_val = expected_dataset.X_val[:,5].copy()
    sex_y_test = expected_dataset.X_test[:,5].copy()

    race_y_train = expected_dataset.X_train[:,4].copy()
    race_y_val = expected_dataset.X_val[:,4].copy()
    race_y_test = expected_dataset.X_test[:,4].copy()
    ...
    if is_mask_gender:
        ...
        X_val[:,7]=0
        X_test[:,7]=0
    print ("sex_val",np.unique(X_val[:,4]),
           "sex_train",np.unique(X_train[:,4]),
    ...
    classes_list=[1,1,1]
    in_channel_=len(expected_dataset.X_train[0])
    model= Net(in_channel=in_channel_,classes_list=classes_list)
    ...
    for epoch in tqdm(range(epochs),total=epochs,desc="train"):
        ...
        for i,(x,y,y_sex,y_race) in enumerate(train_loader):
            x= x.to(device)
            ...
            yhat_att_race = yhat_dict[2]#["att2"]
            ...
            loss_collect.append([loss1.item(),loss_sex.item(),loss_race.item()])
        ...
        if epoch%5==0 or epoch==epochs-1:
            ...
            loss_collect_dict =  zip(loss_collect,["task","sex","race"][:len(loss_collect)] )
    ...
    model_keras = keras.Sequential([
        keras.layers.Dense(50, activation="relu", input_shape=expected_dataset.X_train.shape[1:]),
    ...
    convert_keras_model = model.convert_torch_2_keras(Model_keras=model_keras)
    convert_keras_model.save("models/multitask_models/compas_scores.h5")
    print ("=======")
    print ("========fairness==")
    import sys
    import hashlib
...
if __name__ == "__main__":
    ...
    parser.add_argument(
     "--dataset", default="pre_compas_scores" ,
        )
    ...
    elif args.dataset=="pre_compas_scores":
        from preprocessing import pre_compas_scores as expected_dataset
        print("pre_compas_scores")
        print(expected_dataset.protected_attribs)
        print(expected_dataset.X_train[0])
    else :
