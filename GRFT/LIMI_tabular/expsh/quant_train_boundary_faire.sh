...
for model_dir in ${root}train_dnn/faire_quant_models/adult*; do
    if [ -d "$model_dir" ]; then
...
# # 遍历 models_flip 目录下所有以 adult 开头的子文件夹
for model_dir in ${root}train_dnn/faire_quant_models/bank*; do
    if [ -d "$model_dir" ]; then
        label_file="${model_dir}/labels.npy"
        score_file="${model_dir}/predict_scores.npy"
...
# # 遍历 models_flip 目录下所有以 adult 开头的子文件夹
for model_dir in ${root}train_dnn/faire_quant_models/german*; do
    if [ -d "$model_dir" ]; then
        label_file="${model_dir}/labels.npy"
        score_file="${model_dir}/predict_scores.npy"
...
# # 遍历 models_flip 目录下所有以 adult 开头的子文件夹
for model_dir in ${root}train_dnn/faire_quant_models/compas*; do
    if [ -d "$model_dir" ]; then
        label_file="${model_dir}/labels.npy"
        score_file="${model_dir}/predict_scores.npy"
...
# # 遍历 models_flip 目录下所有以 adult 开头的子文件夹
for model_dir in ${root}train_dnn/faire_quant_models/lsac*; do
    if [ -d "$model_dir" ]; then
        label_file="${model_dir}/labels.npy"
        score_file="${model_dir}/predict_scores.npy"
