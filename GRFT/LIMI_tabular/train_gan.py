...
import numpy as np
from utils.utils_gan import census, credit, bank,compas,lsac, meps


def _parse_args():
    parser = argparse.ArgumentParser(description="CTGAN Command Line Interface")
    parser.add_argument("--exp_name", type=str, default="cencus")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Number of rows to train. Defaults to the training data size",
    )
    ...
    parser.add_argument("--output_train", help="Path of the output file")
    return parser.parse_args()


def load_discrete_columns(args):
    if "census" in args.exp_name:
        return census.discrete_columns
    elif "credit" in args.exp_name:
        return credit.discrete_columns
    elif "bank" in args.exp_name:
        return bank.discrete_columns
    elif "compas" in args.exp_name:
        return compas.discrete_columns
    elif "lsac" in args.exp_name:
        return lsac.discrete_columns
    elif "meps" in args.exp_name:
        return meps.discrete_columns
...
def main():
    ...
    ## control the length of data
    if args.num_train:
        data = data.sample(n=args.num_train, random_state=1)
        data.to_csv(args.output_train, index=False)
    discrete_columns = load_discrete_columns(args)
