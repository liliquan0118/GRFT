...
import os
import sys
sys.path.append("/data/d/liliquan/fairness/tabular_fairness/backup/evaluation")
import numpy as np
...
if __name__ == '__main__':
    ...
    for ROUND in range(1,11):
        ...
        t1 = time.time()
        ids, _, total_iter,g_id, g_iter, global_time = individual_discrimination_generation(dataset_module.X_train, id_seeds, protected_attribs, constraint, model, decay, l_num, l_num+1, max_iter=10, s_g=1.0, s_l=1.0, epsilon_l=1e-6)
        t2 = time.time()
        m_name=m_path.replace("/","_")
        np.save('discriminatory_data/' + m_name+"_"+benchmark + '_ids_EIDIG' + '.npy', ids)
        num_ids = len(ids)
        global_num_ids = len(g_id)
        print("EIDIG",benchmark,m_path,num_ids,t2-t1,global_num_ids,global_time,ROUND)
        with open(root_dir+filename, 'a+') as save_file:
            print("EIDIG",benchmark,m_path,num_ids,t2-t1,global_num_ids,global_time,ROUND,file=save_file)